...
EBBRT_PUBLISH_TYPE(, PingPong);

using namespace ebbrt;

// We don't store anything in the GlobalIdMap, just return the EbbRef
EbbRef<PingPong> PingPong::Create(EbbId id) { return EbbRef<PingPong>(id); }

// This Ebb is implemented with one representative per machine
PingPong& PingPong::HandleFault(EbbId id) {
  {
    ...
    if (found) {
      auto& rep = *boost::any_cast<PingPong*>(accessor->second);
      EbbRef<PingPong>::CacheRef(id, rep);
      return rep;
    }
  }
  ...
  PingPong* rep;
  {
    ...
    if (unlikely(!created)) {
      ...
      rep = boost::any_cast<PingPong*>(accessor->second);
    } else {
...
PingPong::PingPong(EbbId ebbid) : Messagable<PingPong>(ebbid) {}

// We need to store promises for each Ping and therefore also need an identifier
// for each Ping
ebbrt::Future<void> PingPong::Ping(Messenger::NetworkId nid) {
  uint32_t id;
...
void PingPong::ReceiveMessage(Messenger::NetworkId nid,
